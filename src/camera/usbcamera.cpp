...
: open_name_(open_name), quit_(false), ok_(false), queue_(1), open_count_(0)
{
    ...
    image_width_ = yaml_read<double>(yaml, "image_width");
    image_height_ = yaml_read<double>(yaml, "image_height");
    usb_exposure_ = yaml_read<double>(yaml, "usb_exposure");
    usb_frame_rate_ = yaml_read<double>(yaml, "usb_frame_rate");
    usb_gamma_ = yaml_read<double>(yaml, "usb_gamma");
    usb_gain_ = yaml_read<double>(yaml, "usb_gain");
    try_open();

    // 守护线程
    daemon_thread_ = std::thread{[this] {
        // logger()->info("daemon thread start");
        while (!quit_) {
            ...
            if (open_count_ > 20) {
                logger()->warn("Give up to open {} USB camera", this->device_name);
                quit_ = true;
                ...
                if (capture_thread_.joinable()) {
                    logger()->warn("Stopping capture thread");
        ...
        // logger()->info("daemon thread exit");
    }};
}
...
USBCamera::~USBCamera()
{
    ...
    if (capture_thread_.joinable()) capture_thread_.join();
    logger()->info("USBCamera destructed.");
}
...
cv::Mat USBCamera::read()
{
    std::lock_guard<std::mutex> lock(cap_mutex_);
    if (!cap_.isOpened()) {
        logger()->warn("Failed to read {} USB camera", this->device_name);
        return cv::Mat();
    }
    cap_ >> img_;
    return img_;
...
void USBCamera::open()
{
    ...
    logger()->info("USBCamera fps:{}", cap_.get(cv::CAP_PROP_FPS));
    // logger()->info("USBCamera gamma:{}", cap_.get(cv::CAP_PROP_GAMMA));

    // 取图线程
    capture_thread_ = std::thread{[this] {
        ok_ = true;
        std::this_thread::sleep_for(50ms);
        logger()->info("[{} USB camera] capture thread started ", this->device_name);
        while (!quit_) {
            ...
            if (!success) {
                logger()->warn("Failed to read frame, exiting capture thread");
                break;
            }
